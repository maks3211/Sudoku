...
continuie=false;
   ...
   wznow_button = new QPushButton("Wznów", widget1);

   stat_button = new QPushButton("Statystyki", widget1);
   wyjdz_button = new QPushButton("Wyjdź", widget1);
...
licznik->hide();
  ...
  wznow_button->setGeometry((800-(wznow_button->width()))/2 , 50,80,50);
...
stat_button->move((800-buttonWidth)/2 , 250);
...
//strona 2
    ...
    QLabel *napis3 = new QLabel("TEKST 3" , this);
    QPushButton *guzik2 = new QPushButton("Dalej");
 ...
 //Dif_level *poziomik = new Dif_level(this);
    poziomik = new Dif_level(this);
    poziomik->move(nowa_gra_button->pos().x(),nowa_gra_button->pos().y()+50);
 poziomik->raise();


    connect(wstecz, SIGNAL(clicked()) , this , SLOT(Nowe_okno()));
    connect(guzik2, SIGNAL(clicked()) , this , SLOT(Nowe_okno()));
...
//connect(stat_button, &QPushButton::clicked , gra, &Gra::Load);
//connect(gra, &Gra::was_saved, this, &Main_Window::Start_from_file); // zrobnic lambde do pokazania guzika

connect(gra,&Gra::was_saved,this, [=](){ wznow_button->show();
    stan=2;
continuie=true;});
...
connect(wstecz,&QPushButton::clicked,gra,&Gra::Save);
 //connect(gra, &Gra::was_saved, this, &Main_Window::Start_from_file);
        // connect(poziomik,SIGNAL(chosen()), group, SLOT(start()));
//connect(nowa_gra_button, &QPushButton::clicked,poziomik, &Dif_level::show_window);
nowa_gra_button->raise();
}
...
void Main_Window::Nowe_okno() // Zmiana menu
{
