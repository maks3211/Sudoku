...
Sudoku::Sudoku(std::vector<std::vector<int>>& removed_board,std::vector<std::vector<int>>& full_board, int difficulty)
{
    ...
    std::cout << "/n Rozmiar: " << full_board.size() << "  ."<<std::endl;
...
/*
void Sudoku::Remove(vector<vector<int>>& board, int difficulty)
{
    int doUsuniecia = 0;
    int usnieto = 0;
    switch (difficulty)
     ...
     while (usnieto < doUsuniecia) //pkt 1
        {
         ...
         //if (board[row][col] != 0)  //nie trzeba juz sprawdzac bo brane sa kolejne wartosci z tej tablicy
         //{
